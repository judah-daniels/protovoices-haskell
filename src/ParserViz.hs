...
graphDiag'
  ...
  -> GraphOpts i b
  -> PVDiagram b
graphDiag' (PVGraph graph ntgraph _) opts =
  centerXY
 ...
 where
  ...
  aopts nt v1 v2 =
    with
      ...
      %~ lw 1
      &  shaftStyle
      %~ (if nt then dashingG [0.3, 0.2] 0 else id)
  conn nt (v1, v2) =
    connectOutside' (aopts nt v1 v2) (nodeName v1) (nodeName v2)
  gridy y = mkP2 minx y ~~ mkP2 maxx y # lw 1 # lc lightgrey -- (if y `elem` [0,2,4,5,7,9,11] then grey else  lightgrey)
  gridx x = mkP2 x (ptoy minp) ~~ mkP2 x (ptoy maxp) # lw 1 # lc lightgrey
...
sumGraphDiag g m = frame 1 $ graphDiag' g (def { goEdgeOpts = edgeOpts })
 where
  vals   = sqrt . fromIntegral <$> m
  factor = 3 / (sum vals / fromIntegral (M.size vals))
  edgeOpts e opts = opts & shaftStyle %~ lw w & headLength .~ (5 * w)
